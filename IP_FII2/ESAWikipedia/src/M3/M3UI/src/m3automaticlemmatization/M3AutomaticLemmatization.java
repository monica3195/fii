...
public class M3AutomaticLemmatization{
    ...
    public void run() {
        File inputDir=new File(System.getProperty("user.dir")+sep+"M3"+sep+"2.automaticLemmatizationInput");
		File outputDir=new File(System.getProperty("user.dir")+sep+"M3"+sep+"automaticLemmatizationOutput");
		...
		if(inputDir.exists() && inputDir.isDirectory()){
			Path dirs = Paths.get(System.getProperty("user.dir")+sep+"M3"+sep+"2.automaticLemmatizationInput");
			try(DirectoryStream<Path> dirStream = Files.newDirectoryStream(dirs)){
				for(Path dir:dirStream){
					...
					Process p = r.exec(command+"M3"+sep+"2.automaticLemmatizationInput"+sep+
							dir.getFileName()+" M3"+sep+"automaticLemmatizationOutput"+sep+
			...
			System.out.println("fisierul \'2.automaticLemmatizationInput\' nu exista sau nu este director.");
		}
    }
}
