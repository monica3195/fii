...
public class XMLHandler extends DefaultHandler {
	...
	public XMLHandler(File f, String nume) {
		...
		if(f.getName().equals("automaticLemmatizationOutput"))
			output = new File(System.getProperty("user.dir") + sep + "M3" + sep + "lemmaProcessingOutput");
		else
			output = new File(System.getProperty("user.dir") + sep + "M3" + sep + "lemmaProcessingOutput" + sep + f.getName());
		if (!output.exists()) {
			output.mkdirs();
